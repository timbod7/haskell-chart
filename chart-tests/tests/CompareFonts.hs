...
render :: FilePath -> Int -> Int -> CBProgram () -> IO ()
render f w h m = do
...
renderCairo :: (Int, Int) -> CBProgram () -> IO (C.Render ())
renderCairo (w,h) m = do
  return $ BC.runBackend (BC.defaultEnv bitmapAlignmentFns) m

renderDiagramsCairo :: (Int, Int) -> CBProgram () -> IO (C.Render ())
