...
extraSpace :: PieChart -> CBProgram (Double, Double)
extraSpace p = do
...
minsizePie :: PieChart -> CBProgram (Double, Double)
minsizePie p = do
    (extraw,extrah) <- extraSpace p
    return (extraw * 2, extrah * 2)

renderPie :: PieChart -> (Double, Double) -> CBProgram (PickFn a)
renderPie p (w,h) = do
    (extraw,extrah) <- extraSpace p
    ...
    where
        ...
        paint center radius a1 (color,pitem) = do
            ...
            where
                pieLabel :: String -> Double -> Double -> CBProgram ()
                ...
                pieSlice :: Point -> Double -> Double -> AlphaColour Double -> CBProgram ()
